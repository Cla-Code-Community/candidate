  ...
  return (
    <div className="bg-white dark:bg-[#0f131a] p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm theme-transition">
      <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
        ...
        <span className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-500 dark:bg-slate-800 dark:text-slate-300">
        ...
        {scrapers.length === 0 ? (
          ...
          scrapers.map((scraper) => (
            <ScraperCard
              key={scraper.id}
              scraper={scraper}
              isStarting={startingScraperId === scraper.id}
              onToggle={onToggle}
            />
          ))
        )}
